...
parser.add_argument('--dataRoot', default='./POKEMON/', help='path to dataset')
...
parser.add_argument('--outDir', default='./outputs', help='folder to output images and model checkpoints')
...
transform=transforms.Compose([
            transforms.Scale(opt.imageSize + 16),
            transforms.RandomCrop(opt.imageSize),
            transforms.ToTensor(),
            transforms.Normalize((0.5,0.5,0.5), (0.5,0.5,0.5)), # bring images to (-1,1)
        ])

pokemon_dataset = PokemonDataset(opt.dataRoot, transform)
...
dataloader = torch.utils.data.DataLoader(pokemon_dataset, batch_size=opt.batchSize,
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader):
        ...
        errD = criterion(True, output_real, output_fake)
        errD.backward()
        ...
        errG = criterion(False, None, output_fake)
